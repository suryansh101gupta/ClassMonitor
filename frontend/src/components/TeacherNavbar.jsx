...
const TeacherNavbar = () => {
  ...
  const { teacherData, backendUrl, setTeacherData, setIsLoggedin, getTeacherData, isLoggedin } = useContext(AppContext);
  ...
  const sendVerificationOtp = async () => {
    try {
      ...
      const { data } = await axios.post(backendUrl + '/teachers/send-teacher-verify-otp');
      if (data.success) {
  ...
  return (
    <div className='w-full flex justify-between items-center p-4 sm:px-8 sticky top-0 z-50 bg-[#F4F0E6] border-b-4 border-[#111111] shadow-[0_4px_0px_#111111] mb-8'>
      ...
      {teacherData ? (
            ...
            >
              <div className='bg-white border-2 border-[#111111] shadow-[6px_6px_0px_#111111] rounded-lg overflow-hidden flex flex-col'>
                ...
                <ul className='list-none m-0 p-0 flex flex-col'>
                  <li
                    onClick={() => { navigate('/teacher-dashboard'); setShowDropdown(false); }}
        ...
        /* Teacher is logged in but teacherData is still loading — show spinner avatar */
        <div className='w-12 h-12 flex justify-center items-center rounded-full bg-[#F4F0E6] border-2 border-[#111111] shadow-[3px_3px_0px_#111111]'>
          <div className='w-5 h-5 border-2 border-[#111111] border-t-[#FF5722] rounded-full animate-spin'></div>
        </div>
      ) : (
